...
When you work with scientific assessment, you will encounter the two dominating ways to interpret a probability (SO 5.10.). 
...
Both uncertainty and variability can be represented by probability distributions, as illustrated in the left and central graphs in Figure 1.
...
A probability distribution can be represented by its Cumulative Distribution Function (often abbreviated as CDF). The CDF gives the probability that the quantity ls less than or equal to any specified value.

SHOW CDF FOR A SMOOTH VERSION OF THE PROBABILITY DISTRIBUTION IN THE GD
...
SHOW LINES READING FROM THE CDF STARTING FROM THE x-axis where the probability is highlighted
...
SHOW LINES READING FROM THE CDF STARTING FROM THE y-axis 
...
SHOW LINE WITH COLORED AREA BELOW THE LINE
...
and/or 
the P25–P75 range (within which it is 50% certain the true value lies)
...
SHOW PDF FOR A SMOOTH VERSION OF THE PROBABILITY DISTRIBUTION IN THE GD  
